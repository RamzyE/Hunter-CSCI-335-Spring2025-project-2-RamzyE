...
std::unordered_set<Item> Inventory<Comparator, Tree>::query(const Item &start, const Item &end) const {
    std::unordered_set<Item> range;
    if (Comparator::lessThan(end, start)) {
        return {};
    }

    queryHelper(start, end, items_.root(), range);
    return range;
}
...
void Inventory<Comparator, Tree>::queryHelper(const Item &start, const Item &end, const Node *root, std::unordered_set<Item> &result) const {
    ...
    if (root) {
        bool isMatching = Comparator::leq(start, root->value_) && Comparator::leq(root->value_, end);
        bool overStart = Comparator::leq(start, root->value_);
        bool BelowEnd = Comparator::leq(root->value_, end);
