...
void Inventory<Comparator, std::unordered_set<Item>>::discardEquipped() {
    if (equipped_ != nullptr) { // Deletes the equipped_ item and sets it to nullptr if it isn't empty.
        delete equipped_;
        equipped_ = nullptr;
    }
}
...
bool Inventory<Comparator, std::unordered_set<Item>>::pickup(const Item &target) {
    if (items_.count(target.name_) > 0) {
        return false;
    }
...
Inventory<Comparator, std::unordered_set<Item>>::query(const Item &start, const Item &end) const {
    ...
    for (auto it = items_.begin(); it != items_.end(); ++it) {
        bool isMatching = Comparator::leq(start, *it) && Comparator::leq(*it, end);
        if (isMatching) {
            range.insert(*it);
        }
    }
